...
must_haves:
  ...
  artifacts:
    - path: "backend/tests/pipeline/test_pptx_extractor.py"
      provides: "TDD stubs for PPTX-01..04 covering group recursion, SmartArt flag, notes, tables, master"
...
def simple_pptx(tmp_path_factory):
    ...
    slide = prs.slides.add_slide(blank)
    txBox = slide.shapes.add_textbox(Inches(1), Inches(1), Inches(4), Inches(1))
    txBox.text_frame.text = "Short text"
...
```
  ...
  <done>PPTX test files created in TDD RED state with all required test function names.</done>
</task>
