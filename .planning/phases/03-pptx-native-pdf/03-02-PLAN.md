...
def test_reassemble_pptx_identity_round_trip(simple_pptx):
    ...
    result = reassemble_pptx(prs, segments, translated_map)
...
def test_smartart_write_back_skipped(simple_pptx):
    ...
    result = reassemble_pptx(prs, all_segs, translated_map)
...
def test_reassemble_pptx_bullet_paragraph_preserves_bullet_format(simple_pptx, tmp_path):
    ...
    result_prs = reassemble_pptx(prs, segments, translated_map)
...
def test_overflow_flag_when_scale_below_0_7(tmp_path):
    """PDF-03: when insert_htmlbox returns spare_height < 0, overflow flag is set."""
    import pymupdf
    from app.pipeline.pdf.reassembler import reassemble_pdf
    from app.pipeline.segment import Segment
